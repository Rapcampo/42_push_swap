...
void	resolve_num(t_elem *stack_a, t_elem *stack_b, int index, int stk_size);
int		get_median(t_elem *stack);
void	resolve_stack(t_elem *stack_a, t_elem *stack_b, int stk_size);
...
void	resolve_num(t_elem *stack_a, t_elem *stack_b, int index, int stk_size)
{
	...
	if (get_min_stack(stack_b).num > stack_a[index].num
		...
		r_top(stack_b, get_min_above_thresh(stack_b, stack_a[index].num).index,
			"ra\n", "rra\n");
...
void	resolve_stack(t_elem *stack_a, t_elem *stack_b, int stk_size)
{
	...
	while (index < get_end_stack(stack_b))
	{
		resolve_num(stack_b, stack_a,
			get_best_op_index(stack_b, stack_a, stk_size), stk_size);
		index++;
	}
	r_top(stack_a, get_min_above_thresh(stack_a, stack_b[index].num).index,
		"ra\n", "rra\n");
