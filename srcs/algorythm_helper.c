...
void	tiny_sort(t_elem *stack)
{
	...
	if (get_min_stack(stack).index == start && get_max_stack(stack, -1).index == end)
		return ;
	if (get_min_stack(stack).index == start && get_max_stack(stack, -1).index == end - 1)
	{
		ft_swap(stack, "sa\n");
		shift_up(stack, "ra\n");
	}
...
int	get_op_for_gplace(t_elem *a, t_elem *b, int index)
{
	...
	if (get_min_stack(b).num > a[index].num || get_max_stack(b, - 1).num < a[index].num)
